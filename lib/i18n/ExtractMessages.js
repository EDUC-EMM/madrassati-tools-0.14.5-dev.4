 ...
 *
 * Supported Projects
 * - Madrassati (https://github.com/learningequality/madrassati)
...
const SUPPORTED_PROJECTS = ['madrassati', 'studio'];

/* Madrassati */

// Base path to /home/you/code/madrassati/madrassati
const MADRASSATI_ROOT = path.resolve('./madrassati');
const MADRASSATI_PLUGINS_PATHS = MADRASSATI_ROOT + '/plugins';
const MADRASSATI_CORE = MADRASSATI_ROOT + '/core';
...
const MADRASSATI_CSV_PATH = MADRASSATI_ROOT + '/locale/CSV_FILES/en';
...
function filesPaths() {
  switch (PROJECT) {
    case 'madrassati':
      return [MADRASSATI_CORE, MADRASSATI_PLUGINS_PATHS];
    case 'studio':
      return [STUDIO_ROOT];
    default:
...
function clearCsvPath() {
  const path = PROJECT === 'madrassati' ? MADRASSATI_CSV_PATH : STUDIO_CSV_PATH;
 ...
 * frontend assets. Example: coach, device, etc in Madrassati.
 *
 ...
 * case, the `contentcuration` app is the only one. However, for Madrassati, each plugin
...
// from the filePath in whatever case(s) it is necessary (such as Madrassati)
//
// NP - You'll need to account for your product's namespace here as well
function getNamespace(filePath) {
  return PROJECT === 'madrassati'
    ? _getMadrassatiNamespace(filePath)
    : // Studio has one namespace at the moment
      'contentcuration';
}

// For plugins we take the dir name right after the previously defined MADRASSATI_PLUGINS_PATHS
// variable - otherwise return 'core'
function _getMadrassatiNamespace(filePath) {
  if (filePath.includes(MADRASSATI_PLUGINS_PATHS)) {
    try {
      ...
      const pathAfterPlugins = filePath.split(MADRASSATI_PLUGINS_PATHS)[1];
...
try {
  ...
  if (!HELP) {
    ...
    logging.info(
      '\n\n== Madrassati Tools Frontend Message Extraction ==\n\n',
      'This script is intended to be run directly and is hard-coded to work for specific projects: ',
      SUPPORTED_PROJECTS.join(' and '),
      '\n\n',
      'Example: node ./path/to/madrassati-tools/lib/i18n/ExtractMessages.js --project=madrassati',
      '\n\n',
      '[Options]\n\n',

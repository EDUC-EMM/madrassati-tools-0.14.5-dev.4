...
function getVersion(prompt = false) {
  ...
  // See PR: https://github.com/learningequality/madrassati/pull/3777
  ...
  const command = `python -c "import madrassati; print(madrassati.__version__)" > ${filename}`;
  ...
  if (remainder) {
    ...
    if (userDefinedPrerelease) {
      ...
      // As Madrassati versioning will not give us an extra number of dev releases,

...
const cliLogging = logger.getLogger('Madrassati CLI');

function list(val) {
  return val.split(',');
}

program.version(version).description('Tools for Madrassati frontend plugins');
...
program
  .command('build')
  .description('Build frontend assets for Madrassati frontend plugins')
  .arguments(
    '<mode>',
  ...
  .action(function(mode, options) {
    ...
    const buildLogging = logger.getLogger('Madrassati Build');
    const modes = {
      DEV: 'dev',
      PROD: 'prod',
      I18N: 'i18n',
    ...
    const multi = !options.single && !process.env.MADRASSATI_BUILD_SINGLE && numberOfCPUs > 1;

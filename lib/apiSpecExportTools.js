...
const specFilePath = path.resolve(
  path.join(__dirname, '../../../madrassati/core/assets/src/core-app/apiSpec.js')
);
...
const { madrassatiName } = require('./madrassatiName');

function requireName(pathArray, separator = '.') {
  return ['madrassati'].concat(pathArray.slice(1)).join(separator);
}

let baseAliases = {};

const baseAliasSourcePaths = {
  madrassati_module: path.resolve(__dirname, '../../../madrassati/core/assets/src/madrassati_module'),
  madrassati_app: path.resolve(__dirname, '../../../madrassati/core/assets/src/madrassati_app'),
  content_renderer_module: path.resolve(
    __dirname,
    '../../../madrassati/core/assets/src/content_renderer_module'
  ),
  plugin_data: path.resolve(__dirname, '../../../madrassati/core/assets/src/utils/plugin_data_src'),
...
const baseAliasDistPaths = {
  madrassati_module: path.resolve(baseAliasDistPath, 'madrassati_module'),
  madrassati_app: path.resolve(baseAliasDistPath, 'madrassati_app'),
...
// Assume if madrassati_module is not available on the source path, then we need to use the dist
if (fs.existsSync(baseAliasSourcePaths.madrassati_module + '.js')) {
...
function recurseAndCopySpecObject(specObj, targetPath) {
  ...
  function resolveDependenciesAndCopy({
    ...
    // The source file path must be an absolute or relative path, otherwise it is a library external to Madrassati
    // like vue, vuex etc. Do not copy these. Alternatively it is a madrassati API spec reference.
...
const __builder = {
  checkSrc() {
    /*
     * Function to check that we are in the Madrassati source repo whenever doing any of these build tasks.
     */
    if (!fs.existsSync(specFilePath)) {
      throw new ReferenceError(
        'Attempting to build the API Spec from outside the Madrassati source repo'
      );
    }
  },
  buildApiSpec() {
      ...
      if (typeof obj === 'object') {
        ...
        if (lastKey) {
          ...
          if (!obj.startsWith('/')) {
            ...
            spec[lastKey] = requireName(['madrassati'].concat(pathArray), '/') + path.extname(obj);
          }
        }
      }
    }
  ...
  exportApiSpec(distPath) {
     ...
     * wish to use our component library but do not want to import the entire madrassatiCoreAppGlobal object.
     */
    this.checkSrc();
...
function coreExternals() {
  /*
   * Function for creating a hash of externals for modules that are exposed on the core madrassati object.
   */
  const externalsObj = {
    madrassati: madrassatiName,
  };
  ...
  recurseObjectKeysAndExternalize(apiSpec, [madrassatiName]);
  return externalsObj;
}

function coreAliases() {
  /*
   * Function for creating a hash of aliases for modules that are exposed on the core madrassati object.
   */
  ...
  recurseObjectKeysAndAlias(apiSpec, ['madrassati']);
  return aliasesObj;
}
